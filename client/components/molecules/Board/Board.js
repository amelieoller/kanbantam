import React, { useState, useEffect } from 'react';
import PropTypes from 'prop-types';
import { useDispatch } from 'react-redux';
import { parseISO, formatDistanceToNow } from 'date-fns';
import { push } from 'connected-react-router';

import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import {
  faTrashAlt,
  faBan,
  faPencilAlt,
  faSave,
} from '@fortawesome/free-solid-svg-icons';

import { attemptUpdateBoard, attemptDeleteBoard } from '_thunks/boards';
import ConfirmModal from '_organisms/ConfirmModal';
import ListPage from '_pages/ListPage';

const fromNow = (date) => formatDistanceToNow(parseISO(date), { addSuffix: true });

export default function Board({ id, title, createdAt, updatedAt }) {
  const dispatch = useDispatch();

  const [currentTitle, setCurrentTitle] = useState(title);
  const [edit, setEdit] = useState(false);
  const [confirm, setConfirm] = useState(false);
  const [updatedMessage, setUpdatedMessage] = useState('');
  const [createdMessage, setCreatedMessage] = useState('');

  const updateMessages = () => {
    setUpdatedMessage(updatedAt ? fromNow(updatedAt) : '');
    setCreatedMessage(fromNow(createdAt));
  };

  useEffect(() => {
    updateMessages();
    const interval = window.setInterval(updateMessages, 1000);

    return () => clearInterval(interval);
  }, [updatedAt]);

  const openModal = () => setConfirm(true);
  const closeModal = () => setConfirm(false);
  const updateTitle = (e) => setCurrentTitle(e.target.value);
  const editBoard = () => setEdit(true);

  const cancelEdit = () => {
    setEdit(false);
    setCurrentTitle(title);
  };

  const handleUpdateBoard = () => {
    if (currentTitle) {
      dispatch(attemptUpdateBoard(id, currentTitle)).then(() => setEdit(false));
    }
  };

  const deleteBoard = () => {
    dispatch(attemptDeleteBoard(id));
    dispatch(push('/'));
  };

  return (
    <li className="board box">
      <article className="media">
        <div className="media-content">
          <div className="content">
            <p>
              <small>{`created ${createdMessage}`}</small>
            </p>
            {edit ? (
              <textarea
                className="textarea"
                value={currentTitle}
                onChange={updateTitle}
              />
            ) : (
              <p>{title}</p>
            )}
          </div>

          <nav className="level is-mobile">
            <div className="level-left">
              {!!updatedAt && <small>{`edited ${updatedMessage}`}</small>}
            </div>
            <div className="level-right">
              {edit ? (
                <span
                  className="icon space-right"
                  onClick={handleUpdateBoard}
                  onKeyPress={handleUpdateBoard}
                >
                  <FontAwesomeIcon icon={faSave} size="lg" />
                </span>
              ) : (
                <span
                  className="icon space-right"
                  onClick={editBoard}
                  onKeyPress={editBoard}
                >
                  <FontAwesomeIcon icon={faPencilAlt} size="lg" />
                </span>
              )}
              {edit ? (
                <span className="icon" onClick={cancelEdit} onKeyPress={cancelEdit}>
                  <FontAwesomeIcon icon={faBan} size="lg" />
                </span>
              ) : (
                <span className="icon" onClick={openModal} onKeyPress={cancelEdit}>
                  <FontAwesomeIcon icon={faTrashAlt} size="lg" />
                </span>
              )}
            </div>
          </nav>
        </div>
      </article>

      {/* List Section */}
      <ListPage boardId={id} />
      {/* List Section End */}

      <ConfirmModal confirm={confirm} closeModal={closeModal} deleteItem={deleteBoard} />
    </li>
  );
}

Board.propTypes = {
  id: PropTypes.string.isRequired,
  title: PropTypes.string.isRequired,
  createdAt: PropTypes.string.isRequired,
  updatedAt: PropTypes.string,
};

Board.defaultProps = {
  updatedAt: null,
};
